    ...
    const int depth)
{
    ...
    //Iterate on the spheres. Find the intersections with each of them.
    ...
    if ((sphere->transparency > 0 || sphere->reflection > 0) && depth < MAX_RAY_DEPTH) {
        ...
        Vec3f refraction = 0;
        //Sphere is also transparent
        if (sphere -> transparency){
            float ior = 1.1, eta = (inside) ? ior : 1/ior; //Inside or outside the surface?
            float cosi = nhit.dot(raydir);
            float k = 1 - eta*eta*(1-cosi*cosi);
            Vec3f refdir = raydir*eta+nhit*(eta*cosi-sqrt(k));
            refdir.normalize();
            refraction = trace(phit - nhit*bias, refldir, spheres, depth + 1);
        }
